   ...
   <div class="wp cl"> 
    <div id="pt" class="cl"> 
     <div class="z"> 
      ...
      <em>›</em> 
     	<a href="${path}/${section.fid}.html">${section.fname}</a>
      	<em>›</em> 
     	<a href="${path}/${section.yuming}/1_0_0.html">${section.name}</a>
    ...
    <a href="javascript:;" class="pn pnc poster z" onclick="window.history.go(-1);">返回</a> 
     <ul id="thread_types" class="qttp y cl"> 
      ...
      <c:forEach items="${themeList}" var="theme">
	      <li><a href="${theme.id}">${theme.name}<span></span></a></li> 
      </c:forEach>
     </ul> 
    </div> 
   </div> 
 ...
 <div id="wp" class="wp"> 
   <div id="f_pst" class="bm"> 
    <div class="bm_c">
     ...
     <form method="post" id="form" action="${path}/post/update.do">
      <div id="fastpostreturn" style="margin:-5px 0 5px"></div> 
      <div class="pbt cl"> 
       <div class="ftid"> 
        <select name="themeid"  id="themeid"> 
         <c:forEach items="${themeList}" var="theme">
	         <option value="${theme.id}" <c:if test="${post.themeId==theme.id}">selected="selected"</c:if>  >${theme.name}</option>
         </c:forEach>
       </select>
       </div> 
       <input type="text" id="tzname" name="name" class="px" maxlength="80"  tabindex="11" style="width: 560px;" onchange="sysz();" onkeydown="sysz();" onkeyup="sysz();" value="${post.name }"/> 
      ...
      <div class="cl"> 
       ...
       <div class="hasfsl" id="fastposteditor"> 
        <div class="tedt"> 
			<script id="myEditor" type="text/plain" style="width:1000px;height:240px;">${post.contenthtml}</script>
        </div> 
       </div> 
      </div> 
